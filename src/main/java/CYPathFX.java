...
public class CYPathFX extends Application {
                ...
                if (o instanceof Rectangle) {
                    ...
                    if (event.getEventType() == MouseEvent.MOUSE_ENTERED) {
                        ...
                        if (CYPathFX.this.getFenceOrientation() == Orientation.HORIZONTAL) {
                            Fence fence = new Fence(CYPathFX.this.game.getBoard().getFenceLength(),
                                    Orientation.HORIZONTAL, pStartFenceCoord);
                            if (CYPathFX.this.game.getBoard().isFencePositionValid(fence)) {
                                ...
                                Node n = getNodeFromGridPane(CYPathFX.this.gPane, GridPane.getRowIndex(sourceCell) - 1,
                                        GridPane.getColumnIndex(sourceCell)); // get the upper border of the cell
                                if (n instanceof Line) {
                                    Line l = (Line) n;
                            ...
                            Fence fence = new Fence(CYPathFX.this.game.getBoard().getFenceLength(),
                                    Orientation.VERTICAL, pStartFenceCoord);
                            if (CYPathFX.this.game.getBoard().isFencePositionValid(fence)) {
                                ...
                                Node n = getNodeFromGridPane(CYPathFX.this.gPane, GridPane.getRowIndex(sourceCell),
                                        GridPane.getColumnIndex(sourceCell) - 1); // get the left border of the cell
                                if (n instanceof Line) {
                                    Line l = (Line) n;
                                    l.setStroke(Color.DARKRED);
                                    l.toFront();
    ...
    class ActionButtonHandler implements EventHandler<ActionEvent> {
        ...
        public void handle(ActionEvent event) {
            ...
            CYPathFX.this.resetPossibleCells(CYPathFX.this.game.getBoard().getPawnIdTurn()); // à régler avec le PAC
                                                                                             // pour prendre en compte
                                                                                             // le suivi du jeu

            if (actionButton.getText() == "Move" && CYPathFX.this.game.getBoard()
                    .getPawns(CYPathFX.this.game.getBoard().getPawnIdTurn()).getAvailableFences() > 0) {
    ...
    public void showPossibleCells(int pawnId) {
        LinkedList<Point> possibleMoves = this.game.getBoard()
                .listPossibleMoves(this.game.getBoard().getPawns(pawnId).getPosition());
        ...
        for (Point p : possibleMoves) {
            ...
            if (node instanceof Rectangle) {
                ...
                rec.setOnMouseClicked(e -> {
                    this.game.getBoard().getPawns(pawnId)
                            .setPosition(new Point(GridPane.getColumnIndex(rec) / 2, GridPane.getRowIndex(rec) / 2));
